...
module.exports = {
    ...
    add_flow: async function (appid, max) {
        ...
        if (max > 0) {
            var used = await redis.hget(key, appid);
            if (used) {
                used = Number(used);
            }
            if (used >= max) {
    ...
    get_user: async function (session, app) {
        let sessionKey = "APIBUS:SESSION:" + app + ":" + session;
        let token = await redis.get(sessionKey);
        if (!token) {
            return;
        }
        try {
            let sessionInfo = JSON.parse(token);
            return sessionInfo;
        }
        catch (e) {
